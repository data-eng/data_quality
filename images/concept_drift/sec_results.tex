...
point $k=2$ (Section~\ref{sec:margin}, use (i)).
...
As human-annotator disagreement increases from 0/3 to 3/3, the HAS-margin flag rate rises from $2.56\%$ to $10.22\%$ (a $7.66$ percentage-point
increase), compared to $0.64\%$ to $3.19\%$ for the confidence baseline (a $2.55$ point increase). At total disagreement (3/3), HAS flags
...
Even within this same-image setting, the two scores disagree substantially at the instance level: $1{,}457$ images ($2.9\%$ of the
pool) are flagged as informative by the HAS-margin score while the model remains highly confident on them under the baseline score. These
are precisely the images a confidence-based selection strategy would fail to prioritize for relabelling: the model is not visibly uncertain
