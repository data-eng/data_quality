...
The Landscape Recognition dataset consists of $\approx 12{,}000$ curated, web-scraped images across five categories (\texttt{Coast}, \texttt{Desert}, \texttt{Forest}, \texttt{Glacier}, \texttt{Mountain}). This serves as the reference distribution where the HASeparator learns its hyperspherical prototypes. 

The ARXPHOTO314 dataset comprises $18{,}320$ unconstrained, real-world mobile photographs, characterized by high variability in lighting, framing, and environmental conditions. We map the similar ones to the Landscapes's five classes (Table~\ref{tab:classmap}). This target domain represents a real-world distributional shift where the model encounters geometrically ambiguous samples. Our prioritization pipeline exploits this shift by identifying \textit{high-interest samples} instances where the model’s learned boundaries are weakest—and selecting them for targeted fine-tuning to recover accuracy under limited annotation budgets.
