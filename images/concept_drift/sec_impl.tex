...
\label{sec:cifar10n}
We utilize the CIFAR-10N dataset to validate \textbf{H1}. CIFAR-10N provides human-annotated noise labels for the CIFAR-10 training set, capturing the collective consensus of multiple annotators. Unlike standard datasets, this allows us to quantify the "severity" of an error \textit{i.e., how many humans disagreed with the label}. We use this dataset to demonstrate that our geometric margin $\Delta_i$ acts as a robust proxy for human-verified label noise. By showing that low-margin samples are disproportionately likely to be human-annotated errors, we confirm that the HAS model functions as a reliable error detector without needing explicit ground-truth labels.
...
The Landscape Recognition dataset consists of $ 12{,}000$ curated, web-scraped images across five categories (\texttt{Coast}, \texttt{Desert}, \texttt{Forest}, \texttt{Glacier}, \texttt{Mountain}). This serves as the reference distribution where the HASeparator learns its hyperspherical prototypes. 
