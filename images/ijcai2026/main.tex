...
Initially, we employ a pretrained variational autoencoder (VAE) to project the input data into an informative, compressed latent space, avoiding additional training cost. 
On top of this latent representation, we introduce a lightweight embedding network coupled with a discriminative classification head to enforce feature-space separability across classes.
Finally, we apply a cosine farthest-first coreset selection strategy, which is explicitly biased toward samples lying near class boundaries, in order to construct a compact yet informative coreset of instances.



\subsection{Informative discriminated feature space}
...
Variational autoencoders provide a propabilistic latent representation rather than a simple deterministic latent. 
Here we utilized a pre-trained Stable Diffusion (SD) VAE~\cite{fotiadis-valencia-etal:2022,rombach-blattmann-etal:202}, pretrained on the COCO dataset which encodes the input to a compressed latent $z\in\mathbb{R}^(Bx4x(H/8)x(W/8))$, where $B$ the batch size, $4$ the number of channels, $H$ image's height and $W$ the width.
The VAE is frozen ensuring no additional computational costs.

Although the SD-VAE latent space already holds the most important informations, it is not guaranteed that tha features are also discriminated.
Instead of performing instance selection directly in this raw latent space, we train a lightweight embedding network on top of the VAE latents, along with a discriminative hyperplane-assisted softmax head~\cite{kansizoglou2020haseparator}, to obtain inter-class discrimination and intra-class compactness. 
These discriminative embeddings are then used as the feature space on which we apply our cosine farthest-first coreset selection procedure, which focuses on selecting informative and boundary-biased representatives for each class.
...
Each distilled subset (from 10\% to 90\% of the training data) is then used to train a classifier with the same ConvNet. 
